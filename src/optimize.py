...
def optimize(content_targets, style_target, content_weight, style_weight,
    ...
    """
    style_target为：style图片读取的 np 矩阵.
    """
    ...
    with tf.Graph().as_default(), tf.device('/gpu:0'), tf.Session(config=gpuConfig) as sess:
        style_image = tf.placeholder(
            tf.float32, shape=style_shape, name='style_image')
        style_image_pre = vgg.preprocess(style_image)
        net = vgg.net(vgg_path, style_image_pre)
        style_pre = np.array([style_target])  # 数据类型的转换.
        for layer in STYLE_LAYERS:
            features = net[layer].eval(feed_dict={style_image: style_pre})
            # 应该是以通道数为列，转化为(feature，3)
            features = np.reshape(features, (-1, features.shape[3]))
            # size,返回np的元素个数总和.
            gram = np.matmul(features.T, features) / features.size
            style_features[layer] = gram  # gram.shape (3,3)
            print("gram.shape的形态真的是这个吗，到底有几个:", gram.shape)
            """
            gram:一共显示了五次.
            (64,64)(128,128)(256,256)(512,512)(512,512)
            """
    # precompute content feature
    with tf.Graph().as_default(), tf.device('/gpu:0'), tf.Session(config=gpuConfig) as sess:
        ...
        for epoch in range(epochs):
            ...
            while iterations * batch_size < num_examples:
                ...
                # 等价于： is_last = (epoch == (epochs - 1)) and (iterations * batch_size) >= num_examples
